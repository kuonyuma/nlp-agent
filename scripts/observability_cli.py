...
async def run(args: argparse.Namespace) -> None:
    ...
    if args.command == "overview":
        result = await service.overview(args.days)
    elif args.command == "traces":
        result = await service.traces(
            limit=args.limit, session_id=args.session_id, status=args.status
        )
    elif args.command == "trace":
        result = await service.trace(args.trace_id)
    elif args.command == "sessions":
        result = await service.sessions(args.days, args.limit)
    elif args.command == "usage":
        result = await service.usage(args.days)
    elif args.command == "errors":
        result = await service.errors(args.days, args.limit)
