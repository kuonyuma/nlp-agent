...
export function Feedback({
  ...
  const handleDelete = async (threadId: string) => {
    ...
    if (!window.confirm(`删除 ${target?.display_name || target?.username || threadId} 的反馈？该操作不可恢复。`)) return;
    setDeletingId(threadId);
    setDeleteError("");
    try { await onDelete(threadId); }
  ...
  return <Section title="学生意见反馈" hint="按状态、分类和优先级管理反馈；支持回复、标记已读与状态流转。">
    ...
    <div className="developer-feedback">
      <div className="developer-feedback-list">
        {loading && threads.length === 0 ? <div className="developer-feedback-empty"><RefreshCw className="spin" /><strong>正在加载反馈…</strong></div> : threads.length === 0 && loadError ? <div className="developer-feedback-failed"><Inbox size={20} /><strong>加载反馈失败</strong><p>{loadError}</p><button type="button" onClick={() => void refresh()}>重试</button></div> : threads.length === 0 ? <div className="developer-feedback-empty"><Inbox size={22} /><strong>{search || statusFilter || categoryFilter || priorityFilter ? "没有匹配的反馈" : "暂无反馈"}</strong><p>{search || statusFilter || categoryFilter || priorityFilter ? "调整筛选条件或搜索" : "学生提交的反馈会出现在这里"}</p></div> : <>
          ...
          {threads.map((item) => <div key={item.thread_id} className={`developer-feedback-row ${item.thread_id === selectedId ? "active" : ""}`}>
            <button type="button" className="developer-feedback-row-main" onClick={() => onSelect(item.thread_id)} aria-label={`查看 ${item.display_name || item.username} 的反馈`}><span className="developer-feedback-avatar" aria-hidden>{getInitial(item.display_name || item.username)}</span><span className="developer-feedback-row-text"><span className="developer-feedback-row-name"><strong>{item.display_name || item.username}</strong><span className="developer-feedback-category">{categoryLabel(item.category)}</span><span className="developer-feedback-status" style={{ background: statusColor(item.status) }}>{statusLabel(item.status)}</span><span className="developer-feedback-priority-dot" style={{ background: priorityMeta(item.priority).color }} title={`${priorityMeta(item.priority).label}优先级`} />{item.unread_count > 0 && <b className="developer-feedback-unread">{item.unread_count > 99 ? "99+" : item.unread_count}</b>}</span><small><span className="developer-feedback-username">@{item.username}</span><span className="developer-feedback-dot">·</span><span className="developer-feedback-preview">{item.latest?.body?.trim() || "暂无内容"}</span><span className="developer-feedback-dot">·</span><span className="developer-feedback-time">{formatTime(item.updated_at)}</span></small></span></button>{onDelete && <button type="button" className="developer-feedback-row-delete" aria-label={`删除 ${item.display_name || item.username} 的反馈`} disabled={deletingId === item.thread_id} onClick={() => void handleDelete(item.thread_id)}><Trash2 size={14} /></button>}
          </div>)}
        </>}
      </div>
      <div className="developer-feedback-detail">
        ...
        </> : !activeError && <div className="developer-feedback-detail-empty"><MessageCircle size={20} /><strong>{selected ? "正在读取反馈…" : "选择一个账号查看反馈"}</strong><p>{selected ? "正在加载对话详情" : "从左侧选择用户，查看完整反馈时间线。"}</p></div>}
      </div>
    </div>
...
export function DeveloperWorkspace({ page: routedPage, onNavigate }: { page?: DeveloperPage; onNavigate?: (page: DeveloperPage) => void }) {
  ...
  const content = useMemo(() => {
    ...
    if (page === "feedback") return <Feedback threads={feedbackThreads} total={feedbackTotal} pageSize={FEEDBACK_PAGE_SIZE} offset={feedbackOffset} search={feedbackSearch} loadError={feedbackLoadError} loading={feedbackLoading} selectedId={feedbackSelectedId} onSelect={(threadId) => setFeedbackSelectedId(threadId)} onSearchChange={changeFeedbackSearch} onOffsetChange={setFeedbackOffset} onDelete={deleteFeedback} refresh={refreshFeedback} statusFilter={feedbackStatus} categoryFilter={feedbackCategory} priorityFilter={feedbackPriority} sort={feedbackSort} onStatusFilterChange={changeFeedbackStatus} onCategoryFilterChange={changeFeedbackCategory} onPriorityFilterChange={changeFeedbackPriority} onSortChange={changeFeedbackSort} />;
  ...
  }, [changeFeedbackCategory, changeFeedbackPriority, changeFeedbackSearch, changeFeedbackSort, changeFeedbackStatus, deleteFeedback, feedbackCategory, feedbackLoadError, feedbackLoading, feedbackOffset, feedbackPriority, feedbackSearch, feedbackSelectedId, feedbackSort, feedbackStatus, feedbackThreads, feedbackTotal, page, refreshFeedback, snapshot, snapshotError, load]);
