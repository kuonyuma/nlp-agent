...
function CapacitySparkline({ samples }: { samples: Array<{ ready: number; target: number; deficit: number }> }) {
  ...
  return <svg role="img" aria-label="Warm Pool ready capacity history" viewBox={`0 0 ${width} ${height}`} style={{ width: "100%", maxWidth: width, height }}><polyline fill="none" stroke="currentColor" strokeWidth="2" points={points} /></svg>;
}
