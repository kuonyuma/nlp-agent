...
export function TeacherWorkspace({ page: routedPage, onNavigate }: { page?: TeacherPage; onNavigate?: (page: TeacherPage) => void }) {
  ...
  const load = useCallback(async () => { setLoading(true); setError(""); try { const auth = await ensureAuth(); if (!auth.roles.some((role) => role === "developer" || role === "teacher")) throw new Error("当前账户没有教师权限"); const settings = await api.getSettings(); const selectedWorkspaceId = resolveWorkspaceId(auth, settings.preferences.settings ?? {}); const [overview, catalogResult] = await Promise.all([needsOverview ? api.getTeacherOverview(selectedWorkspaceId) : Promise.resolve(null), api.getTeacherCatalog(selectedWorkspaceId)]); setWorkspaceId(selectedWorkspaceId); setData(overview); setCatalog({ ...catalogResult.catalog, guided_blueprints: catalogResult.catalog.guided_blueprints ?? [] }); } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)); } finally { setLoading(false); } }, [needsOverview]);
  ...
  const content = page === "book" ? <TeacherBookEditor workspaceId={workspaceId} catalog={catalog ?? undefined} onCatalogChange={setCatalog} /> : !catalog || (needsOverview && !data) ? null : page === "topics" ? <TopicCatalogEditor topics={catalog.topics} onChange={(topics) => { setCatalog({ ...catalog, topics }); setSaveMessage(""); }} saveProps={saveProps} /> : page === "exercises" ? <BlueprintCatalogEditor kind="exercise" topics={catalog.topics} blueprints={catalog.exercise_blueprints} onChange={(exercise_blueprints) => { setCatalog({ ...catalog, exercise_blueprints: exercise_blueprints as TeacherCatalog["exercise_blueprints"] }); setSaveMessage(""); }} saveProps={saveProps} /> : page === "reviews" ? <BlueprintCatalogEditor kind="review" topics={catalog.topics} blueprints={catalog.review_blueprints} onChange={(review_blueprints) => { setCatalog({ ...catalog, review_blueprints: review_blueprints as TeacherCatalog["review_blueprints"] }); setSaveMessage(""); }} saveProps={saveProps} /> : page === "guided" ? <GuidedBlueprintCatalogEditor topics={catalog.topics} blueprints={catalog.guided_blueprints} onChange={(guided_blueprints) => { setCatalog({ ...catalog, guided_blueprints }); setSaveMessage(""); }} saveProps={saveProps} /> : page === "questions" ? <QuestionsPage data={data!} /> : page === "reports" ? <ReportsPage data={data!} /> : <Overview data={data!} catalog={catalog} />;
  return <div className="teacher-shell"><aside className="teacher-nav"><div className="teacher-brand"><GraduationCap /><span><strong>NLP 教师空间</strong><small>Teacher workspace</small></span><button type="button" onClick={() => void load()} disabled={loading}><RefreshCw className={loading ? "spin" : ""} size={15} />刷新</button></div><nav>{NAV.map(({ page: itemPage, label, icon: Icon }) => <button className={page === itemPage ? "active" : ""} type="button" key={itemPage} onClick={() => navigate(itemPage)}><Icon size={17} />{label}</button>)}</nav><a href="/"><ChevronLeft size={16} />返回学生模式</a></aside><main className={['teacher-main', page === "book" && "teacher-book-main", isCatalogEditor && "teacher-catalog-main"].filter(Boolean).join(" ")}><header className="teacher-topbar"><div><h1>{PAGE_LABELS[page]}</h1><span>{workspaceId} workspace · 目录修改需保存后生效</span></div><div className="teacher-topbar-actions"><SchoolLogo /></div></header><div className={`teacher-content ${page === "book" ? "teacher-content-book" : isCatalogEditor ? "teacher-content-catalog" : ""}`}>{loading ? <div className="teacher-state"><RefreshCw className="spin" />正在加载教学目录…</div> : error ? <div className="teacher-state error"><AlertCircle /><strong>无法进入教师模式</strong><p>{error}</p></div> : content}</div></main></div>;
}
