import { Bold, BookOpen, ChevronDown, Code2, Eye, EyeOff, FilePlus2, Italic, Link2, List, MessageSquareQuote, PanelLeftClose, PanelLeftOpen, Plus, Quote, Redo2, Save, Trash2, Undo2 } from "lucide-react";
...
function MarkdownEditor({ label, value, onChange, placeholder, inputAriaLabel }: { label: string; value: string; onChange: (value: string) => void; placeholder: string; inputAriaLabel?: string }) {
  ...
  return <section className="teacher-catalog-markdown-editor" aria-label={`${label}编辑器`}>
    <div className="teacher-catalog-editor-toolbar">
      <strong>{label}</strong>
      <div className="teacher-catalog-editor-tools">
        ...
        <span className="teacher-catalog-toolbar-divider" />
    ...
    {preview ? <div className="teacher-catalog-markdown-preview"><MarkdownContent>{value || "暂无内容"}</MarkdownContent></div> : <textarea ref={editorRef} className="teacher-catalog-markdown-textarea" aria-label={inputAriaLabel ?? `${label} Markdown`} value={value} onChange={(event) => update(event.target.value)} onKeyDown={onKeyDown} placeholder={placeholder} />}
    <small className="teacher-catalog-editor-hint">支持 Markdown、代码块和常用快捷键。此处不上传图片，也不生成本页小标题目录。</small>
  </section>;
}

type SaveProps = { saving?: boolean; saveMessage?: string; onSave?: () => void };

function CatalogEditorLayout({ eyebrow, title, description, sidebarTitle, count, search, onSearch, createLabel, onCreate, canCreate = true, directory, selected, selectedTitle, selectedMeta, status, onToggleStatus, onDelete, deleteLabel, children, saveProps }: {
  eyebrow: string;
  title: string;
  description: string;
  sidebarTitle: string;
  ...
  return <div className="teacher-catalog-page">
    ...
    <div className={`teacher-catalog-layout ${directoryCollapsed ? "directory-collapsed" : ""}`}>
      <aside className={`teacher-catalog-directory ${directoryCollapsed ? "collapsed" : ""}`} aria-label={`${sidebarTitle}目录`}>
        <button type="button" className="teacher-catalog-directory-expand" aria-label={`展开${sidebarTitle}目录`} onClick={() => setDirectoryCollapsed(false)}><PanelLeftOpen size={16} /></button>
        <header className="teacher-catalog-directory-header"><div><strong>{sidebarTitle}目录</strong><small>{count} 个项目</small></div><div><button type="button" aria-label={createLabel} onClick={onCreate} disabled={!canCreate}><Plus size={15} /></button><button type="button" aria-label={`收起${sidebarTitle}目录`} onClick={() => setDirectoryCollapsed(true)}><PanelLeftClose size={15} /></button></div></header>
        <label className="teacher-catalog-search"><span aria-hidden="true">⌕</span><input type="search" aria-label={`搜索${sidebarTitle}`} value={search} onChange={(event) => onSearch(event.target.value)} placeholder={`搜索${sidebarTitle}`} /></label>
        <div className="teacher-catalog-directory-list">{directory}</div>
      </aside>
      <main className="teacher-catalog-workspace">
        {selected ? <>
          <header className="teacher-catalog-workspace-header"><div className="teacher-catalog-breadcrumb"><span>{eyebrow}</span><b>›</b><h3>{selectedTitle}</h3><small>{selectedMeta}</small>{status && <StatusPill status={status} />}</div><div className="teacher-catalog-workspace-actions">{status && onToggleStatus && <button type="button" onClick={onToggleStatus}>{status === "enabled" ? <EyeOff size={15} /> : <Eye size={15} />}{status === "enabled" ? "停用" : "启用"}</button>}{onDelete && <button type="button" className="danger" aria-label={deleteLabel ?? `删除当前${sidebarTitle}`} onClick={onDelete}><Trash2 size={15} />删除</button>}{saveProps?.onSave && <button type="button" className="primary" onClick={saveProps.onSave} disabled={saveProps.saving}><Save size={15} />{saveProps.saving ? "正在保存…" : "保存教学目录"}</button>}</div></header>
          <div className="teacher-catalog-workspace-scroll">{children}</div>
...
function TopicDirectory({ topics, selection, query, collapsedTopicIds, onSelect, onToggle, onAddPoint }: { topics: CourseTopic[]; selection: TopicSelection; query: string; collapsedTopicIds: string[]; onSelect: (selection: TopicSelection) => void; onToggle: (topicId: string) => void; onAddPoint: (topicId: string) => void }) {
  ...
  return <>{visibleTopics.map((topic) => {
    ...
    return <section className={`teacher-catalog-directory-topic ${topic.status === "disabled" ? "is-disabled" : ""}`} key={topic.id}>
      <div className="teacher-catalog-directory-topic-heading"><button type="button" className="teacher-catalog-directory-topic-toggle" aria-label={`${expanded ? "折叠" : "展开"}主题 ${topic.name}`} aria-expanded={expanded} onClick={() => onToggle(topic.id)}><ChevronDown size={14} /><span>{topic.name || "未命名主题"}</span><small>{topic.knowledge_points.length}</small></button><button type="button" aria-label={`${topic.name}添加知识点`} onClick={() => onAddPoint(topic.id)}><Plus size={14} /></button></div>
      {expanded && <div className="teacher-catalog-directory-topic-items"><button type="button" className={`teacher-catalog-directory-item topic ${selection.kind === "topic" && selection.topicId === topic.id ? "active" : ""}`} onClick={() => onSelect({ kind: "topic", topicId: topic.id })}><BookOpen size={14} /><span>主题设置</span><StatusPill status={topic.status} /></button>{points.map((point) => <button type="button" className={`teacher-catalog-directory-item ${selection.kind === "point" && selection.pointId === point.id ? "active" : ""}`} aria-label={`选择知识点 ${point.name}`} key={point.id} onClick={() => onSelect({ kind: "point", topicId: topic.id, pointId: point.id })}><span>{point.name || "未命名知识点"}</span><StatusPill status={point.status} /></button>)}</div>}
    </section>;
  })}</>;
}
...
export function TopicCatalogEditor({ topics, onChange, saveProps }: { topics: CourseTopic[]; onChange: (topics: CourseTopic[]) => void; saveProps?: SaveProps }) {
  ...
  const toggleCurrent = () => { if (selectedPoint && selectedTopic) updateTopic({ ...selectedTopic, knowledge_points: selectedTopic.knowledge_points.map((point) => point.id === selectedPoint.id ? { ...point, status: point.status === "enabled" ? "disabled" : "enabled" } : point) }); else if (selectedTopic) updateTopic({ ...selectedTopic, status: selectedTopic.status === "enabled" ? "disabled" : "enabled" }); };
  ...
  return <CatalogEditorLayout eyebrow="COURSE CATALOG" title="主题与知识点" description="维护学生学习范围与智能体可引用的知识边界。所有修改先保存在当前目录草稿，点击右上角保存后通过教师接口同步。" sidebarTitle="主题与知识点" count={topics.reduce((total, topic) => total + 1 + topic.knowledge_points.length, 0)} search={query} onSearch={setQuery} createLabel="新建主题" onCreate={createTopic} directory={<TopicDirectory topics={topics} selection={effectiveSelection} query={query} collapsedTopicIds={collapsedTopicIds} onSelect={setSelection} onToggle={(topicId) => setCollapsedTopicIds((current) => current.includes(topicId) ? current.filter((id) => id !== topicId) : [...current, topicId])} onAddPoint={addPoint} />} selected={Boolean(selectedTopic || selectedPoint)} selectedTitle={currentTitle} selectedMeta={currentMeta} status={currentStatus} onToggleStatus={toggleCurrent} onDelete={() => setDeleteTarget(selectedPoint ? { kind: "point", topicId: effectiveSelection.topicId, pointId: selectedPoint.id, name: selectedPoint.name } : selectedTopic ? { kind: "topic", topicId: selectedTopic.id, name: selectedTopic.name } : null)} deleteLabel={selectedPoint ? "删除当前知识点" : "删除当前主题"} saveProps={saveProps}>
    {selectedPoint && selectedTopic ? <div className="teacher-catalog-editor-content"><div className="teacher-catalog-field-grid"><label>知识点名称<input aria-label="知识点名称" value={selectedPoint.name} onChange={(event) => updateTopic({ ...selectedTopic, knowledge_points: selectedTopic.knowledge_points.map((point) => point.id === selectedPoint.id ? { ...point, name: event.target.value } : point) })} placeholder="例如：缩放点积注意力" /></label><div className="teacher-catalog-info-card"><span>所属主题</span><strong>{selectedTopic.name}</strong><small>主题或知识点状态可在右上角切换</small></div></div><MarkdownEditor key={`point-${selectedPoint.id}`} label="知识点说明" value={selectedPoint.markdown} onChange={(markdown) => updateTopic({ ...selectedTopic, knowledge_points: selectedTopic.knowledge_points.map((point) => point.id === selectedPoint.id ? { ...point, markdown } : point) })} placeholder="用 Markdown 写清概念边界、必须覆盖的内容与容易混淆处。" /></div> : selectedTopic ? <div className="teacher-catalog-editor-content"><div className="teacher-catalog-field-grid"><label>主题名称<input aria-label="主题名称" value={selectedTopic.name} onChange={(event) => updateTopic({ ...selectedTopic, name: event.target.value })} placeholder="例如：Transformer" /></label><div className="teacher-catalog-info-card"><span>目录规模</span><strong>{selectedTopic.knowledge_points.length} 个知识点</strong><small>知识点可从左侧主题行的“＋”直接添加</small></div></div><MarkdownEditor key={`topic-${selectedTopic.id}`} label="主题说明" value={selectedTopic.description} onChange={(description) => updateTopic({ ...selectedTopic, description })} placeholder="说明该主题的学习范围、前置知识和教学目标。" /><div className="teacher-catalog-related-heading"><div><strong>主题下的知识点</strong><small>选择左侧项目即可直接编辑；停用项目会自动排列到目录末尾。</small></div><button type="button" onClick={() => addPoint(selectedTopic.id)}><Plus size={15} />新增知识点</button></div></div> : null}
...
function BlueprintDirectory({ topics, blueprints, selectedId, query, collapsedTopicIds, onSelect, onToggle }: { topics: CourseTopic[]; blueprints: Blueprint[]; selectedId: string; query: string; collapsedTopicIds: string[]; onSelect: (id: string) => void; onToggle: (id: string) => void }) {
  const normalizedQuery = query.trim().toLocaleLowerCase();
  ...
  return <>{visibleTopics.map((topic) => {
    ...
    return <section className={`teacher-catalog-directory-topic ${topic.status === "disabled" ? "is-disabled" : ""}`} key={topic.id}><div className="teacher-catalog-directory-topic-heading"><button type="button" className="teacher-catalog-directory-topic-toggle" aria-label={`${expanded ? "折叠" : "展开"}主题 ${topic.name}`} aria-expanded={expanded} onClick={() => onToggle(topic.id)}><ChevronDown size={14} /><span>{topic.name || "未命名主题"}</span><small>{blueprints.filter((item) => item.topic_id === topic.id).length}</small></button></div>{expanded && <div className="teacher-catalog-directory-topic-items">{visiblePoints.map((point) => <div className="teacher-catalog-blueprint-point" key={point.id}><span>{point.name || "未命名知识点"}</span>{sortedLast(blueprints.filter((item) => item.topic_id === topic.id && item.knowledge_point_id === point.id && blueprintMatches(item, topic, normalizedQuery)), (item) => item.status === "disabled").map((item) => <button type="button" className={`teacher-catalog-directory-item ${selectedId === item.id ? "active" : ""}`} key={item.id} onClick={() => onSelect(item.id)}><FilePlus2 size={13} /><span>{item.name || "未命名蓝图"}</span><StatusPill status={item.status} /></button>)}</div>)}</div>}</section>;
  })}</>;
}
...
function BlueprintEditor({ kind, topics, blueprints, exerciseBlueprints = [], onChange, saveProps }: { kind: BlueprintKind; topics: CourseTopic[]; blueprints: Blueprint[]; exerciseBlueprints?: ExerciseBlueprint[]; onChange: (items: Blueprint[]) => void; saveProps?: SaveProps }) {
  ...
  const toggleStatus = () => { if (selected) update({ ...selected, status: selected.status === "enabled" ? "disabled" : "enabled" }); };
  ...
  return <CatalogEditorLayout eyebrow={kind === "guided" ? "GUIDED MODE" : kind === "review" ? "REVIEW BLUEPRINT" : "EXERCISE BLUEPRINT"} title={blueprintName(kind)} description={kind === "guided" ? "为智能体定义分步追问和启发路径；教师保存后，学生引导模式会使用启用的蓝图。" : `为每个知识点维护${kind === "review" ? "复习" : "练习"}生成规则。右侧编辑区支持 Markdown，保存后由后端校验并同步到学生端。`} sidebarTitle={blueprintName(kind)} count={blueprints.length} search={query} onSearch={setQuery} createLabel={`新建${blueprintName(kind)}`} onCreate={create} canCreate={activeTopics.length > 0} directory={<BlueprintDirectory topics={topics} blueprints={blueprints} selectedId={selected?.id ?? ""} query={query} collapsedTopicIds={collapsedTopicIds} onSelect={setSelectedId} onToggle={(topicId) => setCollapsedTopicIds((current) => current.includes(topicId) ? current.filter((id) => id !== topicId) : [...current, topicId])} />} selected={Boolean(selected)} selectedTitle={selected?.name || `未命名${blueprintName(kind)}`} selectedMeta={selectionMeta} status={selected?.status} onToggleStatus={toggleStatus} onDelete={() => selected && setDeleteTarget(selected)} deleteLabel={`删除当前${blueprintName(kind)}`} saveProps={saveProps}>
