...
function BucketCard({ bucket }: { bucket: QuotaBucketSnapshot }) {
  ...
  return <article className="quota-bucket-card">
    <div className="quota-bucket-heading"><span>{bucket.owner_type === "workspace" ? "工作空间" : "用户"} · {bucket.bucket_type === "daily" ? "今日" : "本月"}</span><strong>{formatMicro(bucket.remaining_micro)}</strong></div>
...
export function QuotaUsagePage() {
  const { user } = useAuth();
  const workspaceId = user?.workspace_ids.find((item) => item !== "*");
  ...
  const load = useCallback(async () => {
    setError("");
    try {
      const [quotaResult, usageResult] = await Promise.all([api.getQuota(workspaceId), api.getUsage(30)]);
  ...
  return <main className="quota-page">
    <header className="quota-page-header"><div><span className="quota-eyebrow">ACCOUNT RESOURCE</span><h1>额度与用量</h1><p>额度由开发者统一分配。页面只展示当前账号可用的额度、预占和真实消耗。</p></div><button type="button" onClick={() => void load()} disabled={loading}><RefreshCw size={16} className={loading ? "spin" : ""} />刷新快照</button></header>
    ...
    <section className="quota-summary-grid"><article><Coins /><span>当前可用</span><strong>{formatMicro(buckets.reduce((total, item) => total + item.remaining_micro, 0))}</strong></article><article><Activity /><span>30 天已计费用量</span><strong>{formatMicro(usage?.priced_credits_micro)}</strong></article><article><Clock3 /><span>已记录事件</span><strong>{Number(usage?.events ?? 0).toLocaleString("zh-CN")}</strong></article></section>
