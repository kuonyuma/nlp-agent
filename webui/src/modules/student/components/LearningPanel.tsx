...
import type { ChatMessage, LearningContext, SessionLearningMeta } from "@/shared/types";
...
export function LearningPanel({ open, onClose, title, context, meta, messages, onPrompt, onMeta }: {
  open: boolean;
  onClose: () => void;
  title: string;
  ...
  return (
    <aside className={`learning-panel ${open ? "open" : ""}`}>
      ...
      }) : <small>完成一次对话后自动整理</small>}</div>{!!meta.reviewConcepts?.length && <p className="review-note">待复习：{meta.reviewConcepts.join("、")}</p>}</section>
