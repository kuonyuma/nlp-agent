...
export function MonitorApp() {
  ...
  useEffect(() => { if (!overview) return; const protocol = location.protocol === "https:" ? "wss:" : "ws:"; const socket = new WebSocket(`${protocol}//${location.host}/ws/observability`); socket.onopen = () => setLive(true); socket.onclose = () => setLive(false); socket.onmessage = (message) => { const frame = telemetryFrame(message.data); if (frame) setEvents((current) => [frame.payload, ...current.filter((item) => item.event_id !== frame.payload.event_id)].slice(0, 300)); }; return () => socket.close(); }, [overview]);
