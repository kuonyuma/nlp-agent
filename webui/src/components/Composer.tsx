...
export function Composer({ disabled, running, centered = false, onSend, onCancel }: {
  disabled: boolean;
  running: boolean;
  centered?: boolean;
  ...
  return <div className={`composer-wrap ${centered ? "centered" : ""}`}>
    ...
    <div className="composer">
      ...
      <div className="composer-toolbar"><span><GraduationCap size={15} />NLP 教学 Agent</span>{running ? <button className="send-button stop" type="button" onClick={onCancel} aria-label="停止生成"><Square size={14} fill="currentColor" /></button> : <button className="send-button" type="button" onClick={submit} disabled={disabled || !content.trim()} aria-label="发送"><ArrowUp size={18} /></button>}</div>
    </div>
    <p className="composer-hint">Enter 发送，Shift + Enter 换行 · 重要结论请结合教材验证</p>
  </div>;
}
