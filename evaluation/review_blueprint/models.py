...
class ReviewSuite(StrictReviewModel):
    id: str = Field(min_length=1)
    name: str = Field(min_length=1)
    description: str = ""


class ReviewBlueprintFixture(StrictReviewModel):
...
class ReviewStudentProfile(StrictReviewModel):
    id: str = Field(min_length=1)
    role: str = Field(min_length=1)
    behavior_rules: list[str] = Field(min_length=1, max_length=20)


class ReviewCase(StrictReviewModel):
    id: str = Field(min_length=1)
    tags: list[str] = Field(default_factory=list)
    student_profile: ReviewStudentProfile
    learning_context: LearningContext
    start_message: str = Field(min_length=1, max_length=1_000)


class ReviewDataset(StrictReviewModel):
    schema_version: Literal["1.0"]
    suite: ReviewSuite
    blueprint_path: str = Field(min_length=1)
    cases: list[ReviewCase] = Field(min_length=5, max_length=5)
    blueprint: ReviewBlueprintFixture


class StudentReviewAnswer(StrictReviewModel):
    content: str = Field(min_length=1, max_length=4_000)


class ReviewTurnSnapshot(StrictReviewModel):
...
class ReviewRunSnapshot(StrictReviewModel):
    case_id: str
    chat_session_id: str
    exercise_session_id: str
    blueprint_id: str
    turns: tuple[ReviewTurnSnapshot, ...]


class ReviewArchitectureResult(StrictReviewModel):
